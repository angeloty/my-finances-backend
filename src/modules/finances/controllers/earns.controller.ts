...
export class EarnsController extends BaseController {
  private service: EarnsService;
  constructor() {
    super();
    this.service = new EarnsService();
  }
  ...
  public async all(
    ...
    try {
      const list: EarnModel[] = await this.service.findAll();
  ...
  public async some(
    ...
    try {
      const element: EarnModel = await this.service.find(request.params.id);
      if (element) {
  ...
  public async add(
    ...
    try {
      const saved: EarnModel = await this.service.create(request.body);
  ...
  public async update(
    ...
    try {
      const saved: EarnModel = await this.service.update(
        request.params.id,
        request.body
      );
